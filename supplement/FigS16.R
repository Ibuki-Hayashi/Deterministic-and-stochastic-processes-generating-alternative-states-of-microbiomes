...
N_plots1 <- function(data,d_list,Medium,ti,sh=sh){
  ...
  for(i in 1:length(Medium)){
    ...
    N_sepmedarrow1[[i]] <- ggplot(pd)+
      geom_star(aes(x=NMDS1, y=NMDS2, starshape=Day, starstroke=0.15),fill=palettes(unique(Medium))[i],size=1.5)+
    ...
    for(j in 1:(length(unique(pd$Day))-1)){
      ...
      N_sepmedarrow1[[i]] <- N_sepmedarrow1[[i]] + geom_link(data=lines,aes(x=x, y=y, xend=xend, yend=yend),size=0.00001,arrow=arrow(angle=7.5,length=unit(0.2,"cm"),type="closed"))
    }
...
Leg_plots <- function(data,d_list,Medium,ti,sh=sh){
  ...
  for(i in 1:length(Medium)){
    pd <- d_list[[i]]
    N_sepmedarrow1[[i]] <- ggplot(pd)+
      geom_star(aes(x=NMDS1, y=NMDS2, starshape=Day, starstroke=0.25),fill=palettes(unique(Medium))[i],size=1.5)+
      ggtitle(as.character(Medium[i])) + xlab("NMDS1") + ylab("NMDS2")+theme_light()
...
# --output
alls1 <- NMDS_1(data=use,exp=exp,Medium=Medium,ti=ti)
alls2 <- N_plots1(data=alls1[[1]],d_list=alls1[[2]],Medium=Medium,ti=ti,sh=sh)
alls3 <- Leg_plots(data=alls1[[1]],d_list=alls1[[2]],Medium=Medium,ti=ti,sh=sh)

piN <- plot_grid(alls2[[1]],alls2[[2]],alls2[[3]],alls2[[4]],
                 alls2[[5]],alls2[[6]],alls2[[7]],alls2[[8]],ncol=2,nrow=4)

pdf("FigS16.pdf",height=10.8,width=7.2)
plot_grid(piN,plot_grid(alls3[[1]],NA,NA,ncol=1,rel_heights=c(0.7,1,1)),ncol=2,rel_widths=c(1,0.3))
dev.off()
