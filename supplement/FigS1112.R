...
pl_list <- list(); plpl_list <- list()
for(k in 1:(length(tax)-1)){
  pl_list[[k]] <- pl(list=L_dlist[[k]],med=Medium,ti=ti,method="bray")
  plpl_list[[k]] <- plpl(mat=pl_list[[k]])
}

#-- output
pdf("FigS11.pdf",height=10.5,width=7.2)
plpl_list[[4]]
dev.off()
pdf("FigS12.pdf",height=10.5,width=7.2)
plpl_list[[3]]
dev.off()
