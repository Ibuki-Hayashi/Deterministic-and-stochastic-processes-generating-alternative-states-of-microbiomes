...
data <- read.csv("table/SupplementaryDataS4.csv")
...
for(i in 1:length(lev)){
  d_la[[i]] <- data[(data[,2]==lev[i])&(data[,4]=="bray")&(data[,5]=="all"),]
  d_lb[[i]] <- data[(data[,2]==lev[i])&(data[,4]=="bray")&(data[,5]!="all"),]
}
...
#-- output
pdf("FigS8.pdf",width=7.2,height=6)
ansB
dev.off()
